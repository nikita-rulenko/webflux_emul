...
public record OrderResponse(
    ...
    String status,
    @JsonProperty(value = "messages", index = 1)
    ...
    public record OrderResponseData(
        @JsonProperty(value = "filters", index = 0)
        OrderResponseFilters filters,
        @JsonProperty(value = "stats", index = 1)
        OrderResponseStats stats,
        @JsonProperty(value = "timestamp", index = 2)
        String timestamp,
        @JsonProperty(value = "orders", index = 3)
        List<Order> orders
    ) {
        @JsonPropertyOrder({"limit", "product_type", "order_id_from", "order_ids"})
        public record OrderResponseFilters(
            @JsonProperty(value = "limit", index = 0)
            Integer limit,
            @JsonProperty(value = "product_type", index = 1)
            String productType,
            @JsonProperty(value = "order_id_from", index = 2)
            Long orderIdFrom,
            @JsonProperty(value = "order_ids", index = 3)
            List<Long> orderIds
        ) {}
        ...
        public record Order(
            ...
            String payDatetime,
            @JsonProperty("promocodes_count")
            Integer promocodesCount,
            ...
            String productType,
            @JsonProperty("combined_pdf_url")
            String combinedPdfUrl,
            @JsonProperty("reserve_key")
            String reserveKey,
            Product product
            // List<Promocode> promocodes - Удалено, т.к. отсутствует в response_body.json на этом уровне
        ) {
